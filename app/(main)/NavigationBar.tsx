...
const Desktop = ({
  ...
  return (
    <nav
      ...
      className={clsx(
        "group relative rounded-full bg-gradient-to-b from-zinc-50/70 to-white/90",
    ...
    >
      <motion.div
        className=" pointer-events-none absolute -inset-px rounded-full opacity-0 transition-opacity group-hover:opacity-100 duration-500"
        style={{ background }}
        aria-hidden="true"
      ></motion.div>
...
const MobileNav = (props: PopoverProps<"div">) => {
  return (
    <Popover {...props}>
      ...
      <Transition.Root>
        ...
        >
          <Popover.Overlay
            className={
              "fixed inset-0 z-50 bg-zinc-800/40 backdrop-blur dark:bg-black/80"
            }
          ></Popover.Overlay>
        </Transition.Child>
        <Transition.Child
        ...
        >
          <Popover.Panel
            focus
            className={
              "fixed inset-x-4 top-8 z-50 origin-top rounded-3xl  bg-gradient-to-b from-zinc-100/75 to-white p-8 ring-1 ring-zinc-900/5 dark:from-zinc-900/50 dark:to-zinc-900 dark:ring-zinc-800"
            }
          >
            <div className="flex flex-row-reverse item-center justify-between">
