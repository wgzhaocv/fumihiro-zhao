...
import { FieldMember, NumberInputProps, set, useFormBuilder } from "sanity";
...
const ReadingTimeInput = (props: NumberInputProps) => {
  ...
  const generate = React.useCallback(() => {
    const bodyMember = members.find((member) => {
      if (member.kind === "field") {
        ...
        // @ts-ignore
        return member.name === schemaType.options?.source ?? "";
      }
      return false;
